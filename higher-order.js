...
//Code Here
let totalPopulation = populations.reduce((acc, val, i) => acc + val);
...
let orderTotals = orders.map(val => val['price'] * (1+ val['tax']));
...
let bobsTotal = purchases.reduce((acc, val, i, arr) => {
  var temp = 0;
  if(val["owner"] === 'Bob') {
    console.log(acc);
